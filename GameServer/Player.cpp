...
Player::Player()
{
	...
	AddHandler(Asset::META_TYPE_SHARE_ROOM_HISTORY, std::bind(&Player::CmdBattleHistory, this, std::placeholders::_1));
...
int32_t Player::CmdBattleHistory(pb::Message* message)
{
...
void Player::SendProtocol(const pb::Message& message)
{
	...
	if (Asset::META_TYPE_SHARE_SAY_HI != meta.type_t()) DEBUG("玩家:{} 发送协议，类型:{} 内容:{}", _player_id, type_t,  message.ShortDebugString());
}
...
//会牌计算方法
//
//总共非重复牌数据, 万饼条:1~9; 风牌:1~4; 箭牌:1~3; 共34张不同的牌
//
//会牌可以任意替换上面任何一张牌,如果多张会牌,则进行上述规则的排列组合
//
...
bool Player::CheckHuiHu(Asset::PaiElement pai, bool check_zimo, bool calculate)
{
	...
	unsigned int SET = cards.size(); //非重复牌数量
	unsigned int COMB = count;
...
{
	if (!_room || !_game) return 0;

	if (!_room->HasHuiPai()) return 0; //不带会儿牌

	const auto& huipai = _game->GetHuiPai();
	
	if (huipai.card_type() <= 0 || huipai.card_value() == 0) return 0;

	const auto& pais = _cards_inhand[huipai.card_type()];
	if (pais.size() == 0) return 0;

	auto count = std::count(pais.begin(), pais.end(), huipai.card_value());
	return count;
}
...
bool Player::CheckAllGangPai(::google::protobuf::RepeatedField<Asset::PaiOperationAlert_AlertElement>& gang_list, const std::map<int32_t, std::vector<int32_t>>& cards_inhand, const std::map<int32_t, std::vector<int32_t>>& cards_outhand, const Asset::PaiElement& pai)
{
	...
	for (auto cards : cards_inhand)
	{
		...
		for (auto card_value : cards.second)
		{
			...
			if (count == 4/* || (count == 3 && _game->IsFanPai(card_type, card_value))) */&& CheckMingPiao(Asset::PAI_OPER_TYPE_ANGANGPAI))
			{
...
void Player::OnGangPai(const Asset::PaiElement& pai, int64_t source_player_id)
{
	...
	if (iit != _cards_outhand.end()) 
	{
		...
		if (count == 3)
		{
			...
			/*if (_game->IsFanPai(pai)) _angang.push_back(pai); //暗杠
...
void Player::OnGangFengPai()
{
	...
	if (!CheckFengGangPai(_cards_inhand)) return;
...
bool Player::OnFaPaiCheck(Asset::PaiOperationAlert& alert, Asset::PAI_OPER_TYPE oper_reason)
{
	...
	if (alert.pais().size()) _game->SetPaiOperation(_player_id, _player_id, alert);

	return alert.pais().size() > 0;
}
